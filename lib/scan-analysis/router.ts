...
const OBVIOUS_SCAM_CUES = [
  ...
  /\bcolis\b|\blivraison\b|postes\s+canada|numéro\s+de\s+suivi|suivi\s+du\s+colis|purolator\b/i,
  /\baccount\s+suspend|verify\s+account|unusual\s+activity\b/i,
  /compte\s+(?:sera\s+)?suspendu|vérifi(?:er|ez)\s+(?:votre\s+)?compte|activité\s+inhabituelle|compte\s+verrouill/i,
  /\bbank|credit\s+union|paypal\b|\brbc\b|\bdesjardins\b|\bscotiabank\b|\btd\s*bank\b|\bbmo\b/i,
  /\bbanque\b|institution\s+financière|caisse\s+populaire|interac\b/i,
  /\b(pay|payment|cash|fee|fine|transfer|send\s+money|wire|gift\s*card)\b/i,
  /\bpay(?:er|ez|e|ons|ent|é|ée|és|ées)\b|paiement|argent|frais\b|\bamende\b|virement|carte(?:-|\s)cadeau/i,
  /\b(login|sign\s*in)\b.*\b(account|details)\b|\blog\s*in\s+to\s+your\s+account\b/i,
  /ouvrir\s+(?:une\s+)?session|connexion\s+requise|mot\s+de\s+passe|code\s+de\s+vérification|confirmer\s+(?:votre\s+)?identité/i,
  /\burgent|immediately|act\s+now|within\s+\d+\s*(hour|minute)\b/i,
  /\burgent\b|immédiatement|agissez\s+maintenant|dans\s+les?\s+\d+\s*(?:heures?|minutes?)/i,
  /\bsuspen\w*|close\s+account|lose\s+access|locked\s+out\b/i,
  /suspendu|suspension|verrouill|perdre\s+l['’]accès|fermeture\s+du\s+compte/i,
  /\bprize|winner|won|congratulations.*won\b/i,
  /\bprix\b|\bgagn(?:é|ée|er)\b|félicitations.*gagn/i,
  /\bjob|work\s+from\s+home|easy\s+money\b/i,
  /\bemploi\b|travail\s+à\s+domicile|argent\s+facile|poste\s+à\s+distance/i,
  /\brecover\s+(your\s+)?(lost\s+)?(crypto\s+)?funds?\b|contact\s+recovery\b|crypto\s+recovery\b/i,
  /récupér(?:er|ation)\s+(?:de\s+)?(?:vos?\s+)?fonds|récupér(?:er|ation)\s+(?:de\s+)?crypto|retrouver\s+vos?\s+fonds/i,
